...
Creating names for the largest communities based on the previous calculation.
```{r}
...
Number of connections between "Moderns" and "Ancients" authors.
```{r}
labelled_important_author_graph %>%
  activate("edges") %>% 
...
Number of authors in each community. Specifically wanting to find a way to look up the packages that these authors frequent in.
```{r}
labelled_important_author_graph %>% 
  activate("nodes") %>% 
  as.tibble() %>% 
...
packages_df %<>% 
  left_join(community_names, by = c("authors" = "Name")) %>%
  na.omit()

packages_df %>% 
  select(package_name, Community) %>% 
  group_by(package_name, Community ) %>% 
  tally() %>% 
  arrange(desc(n)) %>% 
  ungroup() %>% 
  top_n(10) %>% 
  knitr::kable(caption = "Count of Authors by Packages and Community")
  
packages_df %>% 
  group_by(authors, Community) %>% 
  tally() %>% 
  arrange(desc(n)) %>% 
  ungroup() %>% 
  top_n(10) %>% 
  knitr::kable(caption = "Count of Packages by Packages and Community")
```
