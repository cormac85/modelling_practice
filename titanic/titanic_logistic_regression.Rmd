...
glm_fit <- train(form, data = titanic_train,
                 trControl=train_control,
                 method="glm", family="binomial", 
                 metric = tune_metric)
rf_fit <- train(form, data=titanic_train,
                trControl=train_control, 
                method="rf", metric = tune_metric)
svml_fit <- train(form, data=titanic_train,
                  trControl=train_control,
                  method="svmLinear", 
                  metric = tune_metric)
xgb_fit <- train(form, data=titanic_train,
                 trControl=train_control,
                 method="xgbLinear",
                 metric = tune_metric)

models_df <- tibble(model_name =  c("GLM","RF", "SVM", "XGB" ),
                    model = list(GLM = glm_fit,
                                 RF = rf_fit,
                                 SVM = svml_fit,
                                 XGB = xgb_fit))
