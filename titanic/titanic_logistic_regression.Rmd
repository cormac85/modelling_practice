...
algo_list <- c("glm", "rf", "svmLinear", "xgbLinear")
models_df <- 
  tibble(model_name = c("GLM","RF", "SVM", "XGB" ),
...
models_df <-
  ...
  mutate(test_accuracy = 
         ...
         test_specificity = 
           map_dbl(test_confusion_matrix,
               function(x) x$byClass[["Specificity"]])) %>% View()
...
models_df <-
  ...
  mutate(test_confusion_matrix = 
           replace(test_confusion_matrix, model_name == "ENSEMBLE",
                   list(confusionMatrix(models_df$test_pred_class[[5]],
